 ...
 * event.
 */
public class DelimitedTextSerializer implements EventSerializer {

    private static final Logger logger = LoggerFactory.getLogger(DelimitedTextSerializer.class);
    ...
    public void configure(Context context) {
        ...
        inputColNames = StringUtils.split(fieldNames, ",");
        if (inputColNames != null) {
            Arrays.stream(inputColNames).forEach(StringUtils::trimToEmpty);
        }
    ...
    public Event getEvent(RecordEntry entry) {
        ...
        if (data instanceof TupleRecordData) {
            return EventBuilder.withBody(tupleRecord2String((TupleRecordData) data), Charset.forName(charset), headers);
        } else {
            return EventBuilder.withBody(((BlobRecordData) data).getData(), headers);
        }
    }

    private String tupleRecord2String(TupleRecordData data) {
