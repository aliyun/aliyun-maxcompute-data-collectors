...
public class SqlMigrationToolWrapper {
   ...
   */
  public SqlMigrationToolWrapper(String root) {
    this.metaManager = new MetaManager(root);
    this.metaProcessor = new MetaProcessor();
  }

  private List<String> getOdpsDdlList() throws Exception {
    ...
    for (String databaseName : metaManager.listDatabases()) {
      ...
      for (String tableName : metaManager.listTables(databaseName)) {
        try {
          ...
          throw new Exception("Error happened while processing " + databaseName + "." + tableName, e);
        }
      }
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    Option sql = Option
        ...
        .build();
    Option sqlScript = Option
        .builder()
        .longOpt("sql-script")
        .argName("sql-script")
        .hasArg()
        .desc("Path to sql script to check")
        .build();
    ...
    options.addOption(sqlScript);
    options.addOption(settings);

    CommandLineParser parser = new DefaultParser();
    CommandLine cmd = parser.parse(options, args);

    if (cmd.hasOption("sql") && cmd.hasOption("sql-script")) {
      throw new IllegalArgumentException("Cannot specify --sql and --sql-script at the same time");
    }
    ...
    if (validateCommandLine(cmd)) {
      ...
      if (cmd.hasOption("sql")) {
        sqlValue = cmd.getOptionValue("sql");
      } else {
        sqlValue = DirUtils.readFile(Paths.get(cmd.getOptionValue("sql-script")));
      }
      ...
      String cmdLineSyntax =
          "sql-checker -i <metadata dir> -p <default odps project> --sql <sql to check> [--sql-script <path to sql>]"
              + "[--settings k=v k=v ...]";
  ...
  private static boolean validateCommandLine(CommandLine cmd) {
    return (cmd.hasOption("sql") || cmd.hasOption("sql-script")) && cmd.hasOption("input-dir")
