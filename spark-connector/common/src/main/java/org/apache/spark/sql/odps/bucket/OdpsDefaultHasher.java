...
public class OdpsDefaultHasher {
    ...
    public static int hashDate(LocalDate date) {
        return hashLong(date.atStartOfDay(ZoneOffset.UTC).toEpochSecond());
    }
    ...
    public static int hashTimestamp(Instant val) {
        long seconds = val.getEpochSecond();
        int nanos = val.getNano();
        seconds <<= 30;
        seconds |= nanos;
        return OdpsDefaultHasher.hashLong(seconds);
    }

    public static int hashDateTime(long timestamp) {
        return OdpsDefaultHasher.hashLong(Math.floorDiv(timestamp, MICROS_PER_MILLIS));
    }

    public static int hashDateTime(Instant timestamp) {
        return OdpsDefaultHasher.hashLong(timestamp.toEpochMilli());
    }
