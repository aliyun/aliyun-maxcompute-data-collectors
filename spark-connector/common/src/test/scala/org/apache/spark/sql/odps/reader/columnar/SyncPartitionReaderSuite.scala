...
class SyncPartitionReaderSuite extends BaseColumnarReaderSuite {
  ...
  test("SyncPartitionReader handles connection refused exception") {
    val connectionException = new RuntimeException("Connection refused")

    when(mockReaderOptions.isReuseBatch).thenReturn(true)
    when(mockScan.createArrowReader(any(), any())).thenThrow(connectionException)
