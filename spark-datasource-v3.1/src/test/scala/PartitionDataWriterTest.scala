...
/**
  ...
  * @date 2021-12-24
  */
object PartitionDataWriterTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfSingleOverwritePartition = spark.sparkContext
      ...
      .map(f => new TestPartitionData(s"a_test$f", s"b_test$f", p0))
      .toDF
    ...
    val dfSingleAppendPartition = spark.sparkContext
      .parallelize(0 to 5, 2)
      .map(f => new TestPartitionData(s"a_test$f", s"b_test$f", p1))
      .toDF
    ...
    try {
      ...
      val dfDynamicOverwritePartition = spark.sparkContext
        ...
        .map(f => {
          val suffix = f % 2
          new TestPartitionData(s"a_test$f", s"b_test$f", s"$p2-$suffix")
        })
        .toDF
    ...
    try {
      ...
      val dfDynamicAppendPartition = spark.sparkContext
        ...
        .map(f => {
          val suffix = f % 2
          new TestPartitionData(s"a_test$f", s"b_test$f", s"$p3-$suffix")
        })
        .toDF
