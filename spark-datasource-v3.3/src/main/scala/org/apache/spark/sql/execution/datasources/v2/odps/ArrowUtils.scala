...
private[sql] object ArrowUtils {
    ...
    dt match {
      case ArrayType(elementType, containsNull) =>
        ...
        new Field(name, fieldType,
          Seq(toArrowField("element", elementType, odpsTypeInfo, containsNull, timeZoneId, enableArrowExtension)).asJava)
      case StructType(fields) =>
        val fieldType = new FieldType(nullable, ArrowType.Struct.INSTANCE, null)
        new Field(name, fieldType,
          fields.map { field =>
            toArrowField(field.name, field.dataType, odpsTypeInfo, field.nullable, timeZoneId, enableArrowExtension)
          }.toSeq.asJava)
  ...
  def toArrowSchema(schema: Seq[Attribute],
    ...
    new Schema(schema.zip(odpsColumns).map {
      case (field, col) =>
        toArrowField(field.name, field.dataType, col.getTypeInfo, field.nullable, timeZoneId, enableArrowExtension)
    }.asJava)
  }
