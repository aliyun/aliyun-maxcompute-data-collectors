...
import io.trino.plugin.maxcompute.MaxComputeConfig;
import io.trino.plugin.maxcompute.MaxComputeErrorCode;
import io.trino.plugin.maxcompute.MaxComputeTableHandle;
...
public class MaxComputeUtils
{
    ...
    public static Odps getOdps(MaxComputeConfig config)
    {
        if (!StringUtils.allNotNullOrEmpty(config.getAccessId(), config.getAccessKey(), config.getEndPoint(), config.getProject())) {
            throw new TrinoException(MaxComputeErrorCode.MAXCOMPUTE_USER_ERROR, "accessId, accessKey, endpoint and project cannot be null.");
        }
    ...
    public static EnvironmentSettings getEnvironmentSettings(MaxComputeConfig config)
    {
        ...
        if (StringUtils.isNullOrEmpty(config.getQuotaName())) {
            builder.withQuotaName("pay-as-you-go");
        } else {
    ...
    {
        return new TrinoException(MaxComputeErrorCode.MAXCOMPUTE_EXTERNAL_ERROR, e.getMessage() + ", RequestId: " + e.getRequestId(), e);
    }
